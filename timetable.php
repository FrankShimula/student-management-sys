...
if($_SERVER['REQUEST_METHOD'] == "POST"){
  ...
  if(empty($Day) or empty($tname) or empty($sname) or empty($cname ) or empty($st) or empty($et)){
    ...
    $timetablereg = $user->timetable_submit($Day,$tname,$sname,$cname,$st,$et);
    if($timetablereg){
      echo "<h3 style='color:green;margin:0;padding:0;text-align:center'>Timetable updated successfully!! <a style='font-size:20px;color:#8e44ad' href='facultylogin.php'>Login</a></h3>";
    }else{
