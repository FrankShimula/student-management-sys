...
class login_registration_class{
	...
	public function facultyupdatePassword($uname, $newpass, $oldpass){
		global $conn;
		$query = $conn->query("select username from faculty where username='$uname' and password='$oldpass' ");
		$count = $query->num_rows;
		if($count == 0){
			return print("<p style='color:red;text-align:center'>old password not exist.</p>");
		}else{
			$update = $conn->query("update faculty set password='$newpass' where username='$uname' ");
			return print("<p style='color:green;text-align:center'>Password changed successfully.</p>");
		}
	}
	...
	**/
	...
	//function for faculty view announcement
	...
	*/
	...
	public function facultysearch($query){
		global $conn;
		$result = $conn->query("SELECT * FROM faculty WHERE (name LIKE '%".$query."%'
							OR username LIKE '%".$query."%'
								OR contact LIKE '%".$query."%'
									OR email LIKE '%".$query."%') order by name");
		return $result;
	}
	...
	//delete faculty
	public function deletefaculty($name){
		global $conn;
	...
	public function personalattview($date){
		global $conn;
		$res = $conn->query("select at_student.name, attn.*
			from at_student
			inner join attn
...
//Function for faculty inserting for different programs timetable
public function BSM_submit($Day,$tn,$sn,$cn,$st,$et){
	global $conn;
	$ttsub = $conn->query("select tn from bsm where sn='$sn' ");
	$count = $ttsub->num_rows;
	if($count == 0){
		$sql = "insert into bsm(Day,tn,sn,cn,st,et) values('$Day','$tn','$sn','$cn','$st','$et')";
		$result = $conn->query($sql);
		return true;
	}else{
		return false;
...
//Function for admin uploading timetable
public function adminuploadtimetable($Day,$tn,$sn,$cn,$st,$et){
	global $conn;
	$ttsub = $conn->query("select tn from bsm where sn='$sn' ");
	$count = $ttsub->num_rows;
	if($count == 0){
...
//Function for admin posting an announcement
public function post_announcement($msg){
	global $conn;
	#$see = $conn->query("select msg from announcements where msg='$msg' ");
	#$count = $see->num_rows;
	#if($count == 0){
	$sql = "insert into announcements(msg,date) values('$msg',CURRENT_TIMESTAMP)";
	$result = $conn->query($sql);
		return true;
	}
...
public function timetable_display(){
	global $conn;
		$sql = "select * from bsm";
		$result = $conn->query($sql);
		return $result;
